...
import com.ncs.marioapp.databinding.FragmentOTPBinding
...
class OTPFragment : Fragment() {
    ...
    private fun setUpObservers() {
        viewModel.progressStateOTPFragment.observe(viewLifecycleOwner, Observer { isLoading ->
            if (isLoading) {
        ...
        viewModel.errorMessageOTPFragment.observe(viewLifecycleOwner, Observer { error ->
            error?.let {
        ...
        viewModel.otpResultOTPFragment.observe(viewLifecycleOwner, Observer { result ->
            if (result) {
                findNavController().navigate(R.id.action_fragment_o_t_p_to_fragment_set_password)
            }
        })
    }
